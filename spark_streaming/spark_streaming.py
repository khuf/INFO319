...
ds = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("subscribe", "tweets").option("includeTimestamp", "true").option("failOnDataLoss", False).load()
...
df = df.select(trim(regexp_replace(col("parsed_values.full_text"), r'(((https:|http:|www.)\S*)|@(\w){1,15}:|RT)',"")).alias("text"))

# Setup pretrained pipeline for sentiment analysis
sentiment_analysis_pipeline = PretrainedPipeline("analyze_sentiment")
sentiments = sentiment_analysis_pipeline.transform(df.select(col("text")))
...
columns = ["text", "sentiment.result", "sentiment.metadata"]
#sentiments = sentiments.select(*columns).withColumn("combinedResult", arrays_zip(col("result"), col("metadata")))
...
bigram_df = bigram_df.withColumn("text", explode("bigrams")).withColumn("timestamp", lit(current_timestamp())).\
        withWatermark("timestamp", "3 minute").groupBy("text", window(col("timestamp"), "30 seconds")).\
        count().withColumnRenamed("count", "value")


config = {"bootstrap_servers": "localhost:9092", "topic": "trigrams"}
